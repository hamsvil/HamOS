...
import { 
  ...
  BellRing, ShieldCheck, DownloadCloud, UploadCloud, Power, Gauge, Share2, Bug, Sparkles, Code, History, Paperclip, PlusCircle, Server, BrainCircuit, GitBranch, Dices, Dice3, Dice5
} from 'lucide-react';
import { AppView, ChatSession } from '../src/types';
...
export const Sidebar = ({ isOpen, setOpen, currentView, setView, addLog, networkInfo }: any) => {
  ...
  const handleSidebarClick = (viewId: AppView) => {
    setView(viewId);
    setOpen(false);
  }
      ...
      <aside className={`fixed top-0 left-0 h-full w-72 bg-[#020617] z-[701] transform transition-transform duration-700 border-r border-white/5 flex flex-col shadow-[20px_0_100px_rgba(0,0,0,0.9)] ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar mt-2">
          {[
            ...
            { id: AppView.VIRTUAL_ROOM, label: 'VIRTUAL ROOM', icon: Server },
          ].map((item: any, idx) => (
            <div key={idx} onClick={() => handleSidebarClick(item.id)} 
          ...
          <div>
            <div 
              onClick={() => setIsRandomizeOpen(!isRandomizeOpen)}
              className="flex items-center justify-between px-6 py-3 rounded-xl cursor-pointer transition-all text-slate-500 hover:bg-white/5 hover:text-white"
            >
                <div className="flex items-center gap-4">
                    <Dices size={18} />
                    <span className="text-[11px] uppercase font-black tracking-widest">Randomize Tool</span>
                </div>
                <ChevronRight size={16} className={`transition-transform ${isRandomizeOpen ? 'rotate-90' : ''}`} />
            </div>
            {isRandomizeOpen && (
                <div className="pl-10 pr-2 pt-1 pb-2 space-y-1 animate-in fade-in duration-300">
                    <div 
                        onClick={() => handleSidebarClick(AppView.RANDOMIZE_TOOL)}
                        className={`flex items-center gap-4 px-4 py-3 rounded-lg cursor-pointer transition-all ${currentView === AppView.RANDOMIZE_TOOL ? 'bg-orange-600/10 text-orange-400' : 'text-slate-500 hover:bg-white/5 hover:text-slate-300'}`}
                    >
                        <Dice3 size={16} />
                        <span className="text-[10px] font-bold uppercase tracking-wider">Togel</span>
                    </div>
                    <div 
                        onClick={() => handleSidebarClick(AppView.RANDOMIZE_TOOL)}
                        className={`flex items-center gap-4 px-4 py-3 rounded-lg cursor-pointer transition-all ${currentView === AppView.RANDOMIZE_TOOL ? 'bg-orange-600/10 text-orange-400' : 'text-slate-500 hover:bg-white/5 hover:text-slate-300'}`}
                    >
                        <Dice5 size={16} />
                        <span className="text-[10px] font-bold uppercase tracking-wider">Judol</span>
                    </div>
                </div>
            )}
          </div>
          ...
          {[
            ...
            { id: AppView.SOVEREIGN_GATEWAY, label: 'SOVEREIGN GATEWAY', icon: GitBranch },
            { id: AppView.MEMORY_INJECTION, label: 'MEMORY INJECTION', icon: BrainCircuit },
