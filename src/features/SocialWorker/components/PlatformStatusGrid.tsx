...
export const PlatformStatusGrid = () => {
  const [active, setActive] = useState(false);
  const { settings, updateSettings } = useSocialWorkerStore();

  useEffect(() => {
    console.log("PlatformStatusGrid mounted");
  }, []);

  const handleAction = () => {
    setActive(!active);
    console.log("PlatformStatusGrid action triggered");
  };

  return (
    <Card className="p-4 bg-[var(--bg-secondary)] border-[var(--border-color)] shadow-sm">
      <CardHeader className="p-0 pb-2">
        <CardTitle className="text-sm font-bold text-[var(--text-primary)]">PlatformStatusGrid</CardTitle>
      </CardHeader>
      <CardContent className="p-0">
        <p className="text-[10px] text-[var(--text-secondary)] mb-3 leading-tight">
          Module managing latform tatus rid. Provides real-time state and functional handlers.
        </p>
        <Button 
