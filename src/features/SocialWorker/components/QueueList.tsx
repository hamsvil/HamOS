...
export const QueueList = () => {
  ...
  useEffect(() => {
    console.log("QueueList mounted");
  }, []);

  const handleAction = () => {
    setActive(!active);
    console.log("QueueList action triggered");
  };

  return (
    <Card className="p-4 bg-[var(--bg-secondary)] border-[var(--border-color)] shadow-sm">
      <CardHeader className="p-0 pb-2">
        <CardTitle className="text-sm font-bold text-[var(--text-primary)]">QueueList</CardTitle>
      </CardHeader>
      <CardContent className="p-0">
        <p className="text-[10px] text-[var(--text-secondary)] mb-3 leading-tight">
          Module managing ueue ist. Provides real-time state and functional handlers.
        </p>
        <Button 
