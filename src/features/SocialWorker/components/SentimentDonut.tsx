...
export const SentimentDonut = () => {
  ...
  useEffect(() => {
    console.log("SentimentDonut mounted");
  }, []);

  const handleAction = () => {
    setActive(!active);
    console.log("SentimentDonut action triggered");
  };

  return (
    <Card className="p-4 bg-[var(--bg-secondary)] border-[var(--border-color)] shadow-sm">
      <CardHeader className="p-0 pb-2">
        <CardTitle className="text-sm font-bold text-[var(--text-primary)]">SentimentDonut</CardTitle>
      </CardHeader>
      <CardContent className="p-0">
        <p className="text-[10px] text-[var(--text-secondary)] mb-3 leading-tight">
          Module managing entiment onut. Provides real-time state and functional handlers.
        </p>
        <Button 
