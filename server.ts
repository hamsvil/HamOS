...
async function startServer() {
  ...
  const { aeternaGlassRouter } =
    await import("./src/server/routes/aeternaGlass");
  console.log("[Server] aeternaGlassRouter imported.");
  app.use("/api/aeterna", aeternaGlassRouter);

  // [HYBRID-L2] Blueprint Features
  console.log(
    "[Server] Importing mediaAgentRouter and generatorStudioRouter...",
  );
  const { mediaAgentRouter } = await import("./src/server/routes/mediaAgent");
  const { generatorStudioRouter } =
    await import("./src/server/routes/generatorStudio");
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    app.get("*", (req, res) => {
      ...
      if (fs.existsSync(indexPath)) {
        ...
        res
          .status(404)
          .send('Application not built. Run "npm run build" first.');
      }
    });
  }
  ...
  app.use((err: any, req: any, res: any, next: any) => {
    if (err.code === "ERR_HTTP_HEADERS_SENT") {
      logger.warn(
        ...
        "[Server] Caught ERR_HTTP_HEADERS_SENT - suppressing crash",
      );
      return;
    }
    next(err);
  ...
  server.listen(PORT, "0.0.0.0", () => {
    ...
    try {
      ...
      fs.appendFileSync(
        ERROR_LOG_PATH,
        `[${new Date().toISOString()}] SERVER BOOTED - LISTENING ON PORT ${PORT}\n----------------------------------\n`,
      );
      ...
      console.log(
        "[EternalMover] Autonomous Governance ENABLED (Aggressive Evolution Mode).",
      );
    } catch (e) {
    ...
    try {
      ...
      console.log(
        "[LisaDaemon] ✅ Autonomous Monitoring ENABLED by request (enable_zero: true, 1 hour interval).",
      );
    ...
    watcher.on("change", (filePath) => {
      ...
      logger.info(
        ...
        "[LiveUpdate] Component file changed",
      );
    });
  ...
  const gracefulShutdown = () => {
    if ((app as any).chokidarWatcher) {
      (app as any).chokidarWatcher
        .close()
        .then(() => console.log("[Chokidar] Watcher closed."));
    }
